  ...
  if(plot2d_procrustes==TRUE){
    ...
    for(w in c('With','Without')){
      ...
      plot(proc$mshape[,1],proc$mshape[,2],axes=F,asp=1,cex=1.1,col='white',
           ...
           main=paste(w,
                      ' outliers| Procrustes coordinates & overall \n mean shape (blue), median shape (black)'))

      for (j in 1:length(class_w)) {
        points(as.matrix(proc$rotated[,,j]),pch=20,
      ...
      #legend
      legend(c(par("usr")[3]+2*a,par("usr")[4]-2*a),c(ymax+2.3*b,par("usr")[2]),
             legend=desired_levels,col=colorss[which(table(class_w)>0)],
             pch=19,box.lty=0,cex=1.3,bg="transparent",lty=NA,lwd=1,
             ncol=ifelse(length(desired_levels)<=3,length(desired_levels),
                         floor(length(desired_levels)/2)))
    }
  }
  par(mfrow=c(1,1))
